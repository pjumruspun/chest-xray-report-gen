...
def run_one_step_batch(agent, env, n_max_interaction=np.inf, max_iter=3000, end_score=None,
    ...
    if n_max_interaction == np.inf and end_score == None:
        raise ValueError("Must at least gives n_max_interaction or end_score to the function")
    ...
    explorer = rl.explorer.VecOneStepExplorer(n_max_interaction=n_max_interaction, env=env) ##
    score = [0] * agent.batch_size
    episode = 0
    iteration = 0

    print(f"Start training one step discrete for {n_max_interaction} steps ...")
    while True:
        try:
            ...
            if any(dones): 
                ...
                if verbose:
                    print(f"Episode {episode+1} score_last_10: {np.mean(score_history[-10:])}, avg_score: {avg_score:.3f}, best_score: {best_score:.3f}")
            ...
            if avg_score > end_score:
                print(f"Achieved {avg_score} average score (> {end_score} end_score)")
                break
            
            if (iteration + 1) % 100 == 0:
                print(f"Iteration {iteration + 1} Episode {episode+1} score_last_10: {np.mean(score_history[-10:])}, avg_score: {avg_score:.3f}, best_score: {best_score:.3f}")
...
def run_param_search(learning_rates = [5e-3, 1e-3], batch_size=16, end_score=200):
    ...
    for i, lr in enumerate(learning_rates):
        for j in range(1):
            ...
            plt.plot(res['avg_score_history'], label="lr=" + str(lr), color=colors[i])
